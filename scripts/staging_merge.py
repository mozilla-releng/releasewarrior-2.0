...
archiver_url = "https://hg.mozilla.org/build/tools/raw-file/default/buildfarm/utils/archiver_client.py"   # noqa: E501
...
def run_merge(args):
    ...
    subprocess.check_call([
        'python', 'archiver_client.py', 'mozharness',
        '--destination', 'mozharness-central',
        '--repo', 'mozilla-central',
        '--rev', 'default',
        '--debug'
    ])
    # modify mozharness-central/configs/merge_day/central_to_beta.py with the repos
    # from the arguments and use https instead of ssh to clone
    ctobfile = "mozharness-central/configs/merge_day/central_to_beta.py"
    f = open(ctobfile, 'rt')
    data = f.read()
    f.close()
    data1 = data.replace(
        "ssh://hg.mozilla.org/mozilla-central", "https://hg.mozilla.org/" + from_repo_url
    )
    data2 = data1.replace(
        "ssh://hg.mozilla.org/releases/mozilla-beta", "https://hg.mozilla.org/" + to_repo_url
    )
